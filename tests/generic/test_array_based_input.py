...
def test_watermark_basic_application(blank_pdf_writer, watermark_pdf_bytes):
    """
    Test applying a basic watermark page to a document.
    Verifies that the new page count is correct and the page structure is updated.
    """
    watermark_reader = PdfReader(BytesIO(watermark_pdf_bytes))
    watermark_page = watermark_reader.pages[0]

    # Apply the watermark to the existing blank page
...
def test_watermark_cloning_regression_fix(blank_pdf_writer, watermark_pdf_bytes):
    """
    Regression test for the ArrayObject._clone bug.
    Ensures that the merge_page operation completes without an AttributeError.
    """
    watermark_reader = PdfReader(BytesIO(watermark_pdf_bytes))
    watermark_page = watermark_reader.pages[0]
    ...
    try:
        ...
        pytest.fail(f"AttributeError raised during watermarking/cloning: {e}. The fix failed.")
...
def test_add_page_with_array_content_stream(array_content_pdf_bytes):
    """
    Test that PdfWriter.add_page correctly processes a page whose /Contents
    entry is an ArrayObject of content streams, ensuring no errors occur.
    """
