    ...
    for page in reader.pages:
        ...
        for image_id in page._get_ids_image():
            ...
            with open(image.name, "wb") as fp:
                fp.write(image.data)
            print(f"Image extracted to {image.name}")
...
def parse_args(argv):
    ...
    if args.func == subset_cmd:
        if not args.out_filepath:
            ...
            args.out_filepath = f"./subset.{basename(args.pdf_filepath)}"
        args.page_numbers = page_ranges_to_page_numbers(parser, args.page_ranges)
    return args


def page_ranges_to_page_numbers(parser, page_ranges):
    page_numbers = set()
