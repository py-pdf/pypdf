...
def _xobj_to_image(
    ...
    # Get filters
    filter = x_object.get(StreamAttributes.FILTER, NullObject()).get_object()
    ...
    filters = filter[-1] if filter and isinstance(filter, list) else filter
    ...
    extension = None
    if filters in (FT.FLATE_DECODE, FT.RUN_LENGTH_DECODE):
    ...
    elif filters in (FT.LZW_DECODE, FT.ASCII_85_DECODE):
        # I'm not sure if the following logic is correct.
        # There might not be any relationship between the filters and the
        # extension
        if filters == FT.LZW_DECODE:
            image_format = "TIFF"
    ...
    img = _apply_decode(img, x_object, filters, color_space, invert_color)
