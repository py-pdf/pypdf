    ...
    if isinstance(color_space, str):
        ...
        if color_space_str == "/DeviceCMYK" and color_components == 1:
            if original_color_space[1][0] != "/Black":
                logger_warning(
                    "Color %(original_color_space)s converted to Gray. Please share PDF with pypdf dev team",
                    source=__name__,
                    original_color_space=original_color_space[1][0]
                )
            return "L", True
        mode, invert_color = _get_image_mode(
            color_space_str, color_components, prev_mode, depth + 1
        ...
        try:
            ...
            if img.mode == "1":
                ...
                if len(lookup) != (hival + 1) * nb:
                    logger_warning("Invalid Lookup Table in %(obj_as_text)s",
                                    source= __name__,
...
def _xobj_to_image(
    ...
    try:  # temporary try/except until other fixes of images
        ...
        logger_warning("Failed loading image: %(exception)s",
                        source= __name__,
                        exception=exception)
