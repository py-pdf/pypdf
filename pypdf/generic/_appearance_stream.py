...
class TextStreamAppearance(BaseStreamAppearance):
    ...
    def _generate_appearance_stream_data(
        ...
        if font_size == 0:
            ...
            if max_length and len(text) > max_length:
                logger_warning (
                    "Length of text %(text)s exceeds maximum length (%(max_length)s) of field, input truncated.",
                    source=__name__,text=text,max_length=max_length
                )
    ...
    def __init__(
        ...
        if font_resource:
            ...
            logger_warning("Font dictionary for %(font_name)s not found; defaulting to Helvetica.", source=__name__,font_name=font_name)
            font_name = "/Helv"
