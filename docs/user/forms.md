...
In order to ease locating page fields you can use `page.get_pages_using_field`. This methods accepts a field object, that is a *PdfObject* that represents a field (as extracted from `_root_object["/AcroForm"]["/Fields"]`). The method returns a list of pages, because a field can have multiple widgets as mentioned previously (e.g. radio buttons or text displayed on multiple pages).
