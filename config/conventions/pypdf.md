...
Indexed version: see `store/manifests/pypdf.json` (`ref`, `commit`).

Populate this file after indexing. Ground every section in retrieved chunks:

```bash
python scripts/query.py --library pypdf --prompt "module docstring Args public import style"
python scripts/query.py --library pypdf --prompt "deprecated removed replacement do not use"
```
...
Note: if `config/libraries.yaml` excludes `**/tests/**`, upstream test patterns are not indexed — ground test **behavior** from `repos/pypdf/` source search only.
