...
def ListAnnots(pdfS):
    ...
    for pn in range(pdfS.numPages):
        ...
        try:
            ...
            for b in a:
                ...
                if o["/Subtype"] == "/Text":
                    try:
                        o["/P"]  # le champs '/P' etant optionnel on le reconstruit...
                    except:
    ...
    for k, o in lst.items():
        if "/IRT" in o:
            ...
            lst[o.rawGet("/IRT").idnum].irt[o["/M"]] = "%s (%s):\n%s" % (
                o["/T"],
                datetime.strptime(o["/M"][2:10], "%Y%m%d").strftime("%x"),
                t,
            )
...
def FindOutline(Outlines, pa, pe):
    ...
    m = None
    for o in Outlines:
        if (o[1] < pa) or ((o[1] == pa) and (o[2] <= pe)):
            m = o
    return m
...
if "-o" in sys.argv:
    ...
    xlFile = (
        tempFolder
        + "Comments on "
        + os.path.splitext(os.path.split(pdfS.filepath)[-1])[0]
        + ".xlsx"
    )
...
for x in sorted(lst.keys()):
    p = lst[x]
    if "/IRT" in p:
        continue  # the comments with IRT are already present in the original comment irt field, we can ignore this one
    ...
    # print(cont,',',end='')
    if isinstance(p.irt_str, bytes):
        p.irt_str = p.irt_str.replace(b"\r", b"\n").decode("unicode_escape")
    # print(p.irt_str)

    ws.append(
        (
            pdfS.getPageLabel(x[0]),
            "%.0f %%" % pc,
            FindOutline(pdfS.MyOutlines, x[0], x[1])[0],
            auth,
            cont,
            p.irt_str,
        )
